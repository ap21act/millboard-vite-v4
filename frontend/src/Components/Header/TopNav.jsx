...
const TopNav = ({ openCart }) => {
  ...
  return (
    <div className='flex px-12 pt-6 pb-7 border-b-white-background bg-white-nav justify-between'>
      ...
      <a href='/'>
        <img src={Logo} alt='logo' className='h-16 w-72' />
      </a>
      ...
      <div className='flex gap-4 items-center'>
        ...
        <button onClick={openCart} className='relative'>
          ...
          {totalItems > 0 && (
            <div className="absolute -top-2 -right-3 h-4 w-4 flex items-center justify-center rounded-full bg-red-500 text-white text-xs font-bold bg-primary">
              {totalItems}
            </div>
          )}
        </button>
