    ...
    <section className='py-24 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8'>
      ...
      <div>
        {selectedFaqs.map((faq, index) => (
          <div key={index} className={`accordion border border-solid border-black-background p-4 rounded-xl transition duration-500 mb-8 lg:p-4 ${openIndex === index ? 'bg-white-nav border-green' : ''}`}>
            <button className='accordion-toggle group inline-flex items-center justify-between text-left text-lg font-normal leading-8 text-black-background w-full transition duration-500 hover:text-green' onClick={() => handleToggle(index)}>
              <h5>{faq.question}</h5>
              <svg className={`w-6 h-6 text-primary transition duration-500 ${openIndex === index ? 'hidden' : 'block'}`} viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg'>
                <path d='M6 12H18M12 18V6' stroke='currentColor' strokeWidth='1.6' strokeLinecap='round' strokeLinejoin='round' />
              </svg>
              <svg className={`w-6 h-6 text-black-background transition duration-500 ${openIndex === index ? 'block' : 'hidden'}`} viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg'>
                <path d='M6 12H18' stroke='currentColor' strokeWidth='1.6' strokeLinecap='round' strokeLinejoin='round' />
              </svg>
            </button>
            <div className={`accordion-content w-full overflow-hidden pr-4 transition-max-height duration-500 ease-in-out ${openIndex === index ? 'max-h-64' : 'max-h-0'}`}>
              <p className='text-base text-primary font-normal leading-6 pt-3'>
                {faq.answer}
              </p>
            </div>
          </div>
